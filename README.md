...
**Session verification.** Once the data channel opens, each side generates an 8-byte random nonce and sends it to the other. Both sides compute `SHA-256(sorted(nonce1 + nonce2))` and display the first 6 bytes as a short authentication string (SAS). Users are meant to compare it verbally.
...
**Local persistence.** The passphrase is kept in `sessionStorage` only until the data channel opens, then removed. Messages of the current session are mirrored to `localStorage` (key `btw_msgs`) for up to 24 hours so that a refreshed tab can show a read-only view of the previous conversation; that copy is cleared when a new chat starts or when a new session connects. If you need nothing to remain on the device, clear site data for btwinus.com or use a private window.
...
- **Offline guessing of the passphrase from the link alone.** The generated passphrase is three words from a 30-word list plus a four-digit number, roughly 2.4 x 10^8 combinations (about 28 bits). PBKDF2 with 100,000 iterations slows each guess, but an attacker with the link and a GPU can exhaust that space in hours rather than years. Treat the link as sensitive, use short expiry, and consider that this word list is a known weakness we intend to enlarge.
- **A relaying man-in-the-middle, as far as the SAS is concerned.** The SAS is derived from nonces exchanged over the data channel, not from the DTLS certificate fingerprints. An attacker who has already inserted themselves into the handshake and relays both directions would forward the nonces unchanged, and both users would see the same code. Binding the SAS to the DTLS fingerprints is a planned fix. Until then the SAS confirms you share a channel with someone, not that nobody sits in between.
